...
destination_path="$(echo "${source_path}" | tr '/' ' ' | sed 's/paper.pdf/Paper.pdf/' | sed 's/presentation.pdf/Presentation.pdf/')"
...
if [ "${check}" -gt 0 ]; then
  ...
  cp "${source_path}" "${source_dir}/${destination_path}"
  gdrive files update "${samefileID}" "${source_dir}/${destination_path}"
  rm -f "${source_dir}/${destination_path}"
  echo "Done updating."
else
  echo "It doesn't exist. We'll upload the file."
  source_dir="$(dirname "${source_path}")"
  cp "${source_path}" "${source_dir}/${destination_path}"
  gdrive files upload "${source_dir}/${destination_path}"
  rm -f "${source_dir}/${destination_path}"
  echo "Done uploading."
fi
